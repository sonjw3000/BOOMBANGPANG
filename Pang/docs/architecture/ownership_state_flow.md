...
Picking and Packing destination queries evaluate the items that will exist after the transfer (`HasItems + Picked` or `HasItems + Packed`) instead of selecting an `Empty` Rule and relying on later relocation. Picking may target either a physically empty Capsule or a compatible `Inside + Picked` Capsule, so multiple Picking Tasks can Put into one output. Each selected Task retains the Buffer while moving to it; if an earlier Put crosses the outbound threshold, relocation waits for those existing retains. A partial or failed Put releases that Task's retain and replans the remaining carried quantity. Packing keeps its existing physically empty receiving-Capsule policy.
