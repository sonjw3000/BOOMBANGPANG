...
- `IB` and `OB` describe CargoPort-facing transport phases. A standard Capsule becomes `OB` only when the Building threshold is met and a non-empty Rule on an OutboundCargoPort in the same Building accepts its physical or derived `LaunchReady` cargo stage. `CapsuleRelocateCoordinator` owns this combined decision.
...
- Task implementations change physical cargo, ItemStatus, or manifests. Dirty routing evaluation derives the lifecycle of docked standard Capsules instead of each work Task assigning it independently. Item planners release their temporary Buffer retain after each completed Put, then Relocation evaluation runs before the next item-work scheduling pass. Outbound Rule eligibility gates `Inside -> OB`, while temporary port occupancy only delays the relocation match. A relocation Task may set a carried rejected outbound Capsule back to `Inside` before it can be redocked and evaluated.
