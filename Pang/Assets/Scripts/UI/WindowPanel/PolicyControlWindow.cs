...
namespace Assets.Scripts.UI
{
	...
	public class PolicyControlButton : MonoBehaviour
	{
		[SerializeField] private Button button;
		[SerializeField] private PolicyControlWindow targetWindow;
		[SerializeField] private TMP_Text label;
		[SerializeField] private string buttonText = "P";
		...
		private void Awake()
		{
			...
			if (targetWindow == null)
				targetWindow = FindFirstObjectByType<PolicyControlWindow>(FindObjectsInactive.Include);
	...
	public class PolicyControlRuntimeBootstrap : MonoBehaviour
	{
		...
		private void TryInstall()
		{
			...
			if (GetComponentInChildren<PolicyControlWindow>(true) != null)
			{
				installed = true;
				return;
			}

			Transform buttonContainer = transform.Find("UI_Canvas_Main/RightTop_StaticButtons/ButtonContainer");
			Transform panels = transform.Find("UI_Canvas_Main/Panels");
			ZoneControlWindow zoneWindowTemplate = FindFirstObjectByType<ZoneControlWindow>(FindObjectsInactive.Include);
			if (buttonContainer == null || panels == null || zoneWindowTemplate == null)
			{
				Debug.LogWarning("[PolicyControl] Failed to find UI templates for runtime installation.");
				return;
			}

			Transform zoneButtonTemplate = buttonContainer.Find("UI_Button_Active_Zone");
			UIWindow zoneWindowShell = zoneWindowTemplate.GetComponentInChildren<UIWindow>(true);
			if (zoneButtonTemplate == null || zoneWindowShell == null)
			{
				Debug.LogWarning("[PolicyControl] Zone UI templates are incomplete.");
				return;
			}

			GameObject windowHost = new("PolicyControlWindow", typeof(RectTransform));
			windowHost.transform.SetParent(panels, false);
			RectTransform hostRect = windowHost.GetComponent<RectTransform>();
			...
			GameObject clonedWindowShell = Instantiate(zoneWindowShell.gameObject, windowHost.transform, false);
			clonedWindowShell.name = zoneWindowShell.gameObject.name;
			windowHost.AddComponent<PolicyControlWindow>();

			GameObject buttonObject = Instantiate(zoneButtonTemplate.gameObject, buttonContainer, false);
			buttonObject.name = "UI_Button_Active_Policy";
			buttonObject.transform.SetAsLastSibling();

			ZoneControlButton zoneButton = buttonObject.GetComponent<ZoneControlButton>();
			if (zoneButton != null)
				Destroy(zoneButton);

			buttonObject.AddComponent<PolicyControlButton>();
			installed = true;
		}
	}

	public static class PolicyControlRuntimeInstaller
	{
		...
		private static void Install()
		{
			...
			GameObject uiRoot = GameObject.Find("UI_InGame");
			if (uiRoot == null)
				return;

			if (uiRoot.GetComponent<PolicyControlRuntimeBootstrap>() == null)
				uiRoot.AddComponent<PolicyControlRuntimeBootstrap>();
		}
	}
}
